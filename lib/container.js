...
var Container = function ( containerArray ) {
	...
	this.shell = containerArray.shell || '/bin/sh'
}
...
Container.prototype.startAndMountContainer = function ( options, finishedCallback ) {
	var containerId,
		self = this,
		run = spawn( 'docker', ['run', '-d', '-v', global.config.path + ':/app/test', '-w', '/app/test', '-it', self.image, self.shell] )
...
Container.prototype.runTests = function ( options, containerId, finishedCallback ) {
	...
	var exec = spawn( 'sh', ['-c', 'docker exec -ti ' + containerId + ' ' + this.shell + ' -c "[ -f ~/.bashrc ] && source ~/.bashrc; ' + self.testCommand + ' ' + testArgString + ' " 1>&2'], {stdio: 'inherit'} ).on( 'exit', function ( code ) {
		if ( code !== 0 ) {
